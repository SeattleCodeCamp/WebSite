...
namespace OCC.WindowsPhone.ViewModels
{
    public class AnnouncementViewModel : ViewModelBase
    {
        ...
        {
            get { return Get(() => Announcement); }
            private set { Set(() => Announcement, value);}
        }
        public int Id { get; private set; }
        public void Load(int id)
        {
            var announcement = App.MainViewModel.Announcements.FirstOrDefault(t => t.ID == id);
            if (announcement == null)
            {
                MessageBox.Show("Invalid Announcement Id");
                return;
            }
